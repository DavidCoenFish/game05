...
{
     SetStateFlagDirty(UIStateFlag::TDisable);
     Set(in_disable);
}
    ...
    )
{
    ...
    if (true == SetStateFlagBit(UIStateFlag::TDisable, in_disable))
    {
        dirty = true;
    }
    ...
    )
{
    ...
    if (nullptr != _shader_constant_buffer)
    {
        ...
        const bool disable = (0 != (static_cast<int>(GetStateFlag()) & static_cast<int>(UIStateFlag::TDisable)));
