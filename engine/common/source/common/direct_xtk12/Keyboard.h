...
namespace DirectX
{
   ...
   public:
      ...
      KeyboardButtonStateTracker() noexcept { Reset(); }

      void __cdecl Update(const Keyboard::State& state) noexcept;

      void __cdecl Reset() noexcept;

      bool __cdecl IsKeyPressed(Keyboard::Keys key) const noexcept { return pressed.IsKeyDown(key); }
      bool __cdecl IsKeyReleased(Keyboard::Keys key) const noexcept { return released.IsKeyDown(key); }
      bool __cdecl IsDown(Keyboard::Keys key) const noexcept { return lastState.IsKeyDown(key); }

      Keyboard::State __cdecl GetLastState() const noexcept { return lastState; }
