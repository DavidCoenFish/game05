...
#include "pch.h"
#include "DirectXTK12/LinearAllocator.h"
    ...
    , m_device(pDevice)
{
    ...
    for (size_t preallocatePages = 0; preallocateBytes != 0 && preallocatePages < preallocatePageCount; ++preallocatePages)
    {
        ...
        {
           LOG_MESSAGE_WARNING("LinearAllocator failed to preallocate pages (%zu required bytes, %zu pages)\n",
...
LinearAllocatorPage* LinearAllocator::GetNewPage()
{
    ...
    if (FAILED(hr))
    {
        if (hr != E_OUTOFMEMORY)
        {
            LOG_MESSAGE_WARNING("LinearAllocator::GetNewPage resource allocation failed due to unexpected error %08X\n", static_cast<unsigned int>(hr));
        }
        return nullptr;
    }
