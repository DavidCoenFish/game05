...
DeviceResources::DeviceResources(
    ...
    , dxgi_factory_flags(0)
    , custom_command_list_fence_value(0)
    ...
    , target_depth_data(in_target_depth_data)
{
    ...
    if (options&c_allow_tearing)
    {
        ...
        HRESULT hr = dxgi_factory->CheckFeatureSupport(
    ...
    if (SUCCEEDED(device.As(&d3d_info_queue)))
    {
        ...
        D3D12_MESSAGE_ID hide[] =
        {
            D3D12_MESSAGE_ID_MAP_INVALID_NULLRANGE, D3D12_MESSAGE_ID_UNMAP_INVALID_NULLRANGE, 
                D3D12_MESSAGE_ID_EXECUTECOMMANDLISTS_WRONGSWAPCHAINBUFFERREFERENCE}; D3D12_INFO_QUEUE_FILTER filter = {}
                ;
            filter.DenyList.NumIDs = countof(hide);
            filter.DenyList._id_list = hide;
        ...
        graphics_memory = std::make_unique < DirectX::GraphicsMemory > (device.Get());
        if (nullptr == graphics_memory)
        {
        ...
        DX::ThrowIfFailed(device->CreateCommandAllocator(
            ...
            IID_PPV_ARGS(custom_command_allocator.ReleaseAndGetAddressOf())
            ));

        {
            ...
            custom_command_allocator->SetName(name);
        }
        DX::ThrowIfFailed(device->CreateCommandList(
            0,
            D3D12_COMMAND_LIST_TYPE_DIRECT,
            custom_command_allocator.Get(),
            nullptr,
            IID_PPV_ARGS(custom_command_list.ReleaseAndGetAddressOf())
            ));

        {
            ...
            custom_command_list->SetName(name);
        }
        custom_command_list->Close();
        DX::ThrowIfFailed(device->CreateFence(
            0,
            D3D12_FENCE_FLAG_NONE,
            IID_PPV_ARGS(custom_command_fence.ReleaseAndGetAddressOf())
            ));
        custom_command_fence_event.Attach(CreateEventEx(
            nullptr,
            nullptr,
            0,
            EVENT_MODIFY_STATE | SYNCHRONIZE
            ));
        if (!custom_command_fence_event.IsValid())
        {
        ...
        {
            D3D_SHADER_MODEL_6_5}; if (FAILED(device->CheckFeatureSupport(
            ...
            DeviceResources::~DeviceResources()
            {
                ...
                custom_command_allocator.Reset();
                custom_command_list.Reset();
                custom_command_fence.Reset();
                command_queue.Reset();
                screen_size_resources.reset();
            ...
            {
                if (screen_size_resources)
                {
                    screen_size_resources->WaitForGpu(in_command_queue);
                }
                return;
            }
            ...
            {
                if (command_queue && custom_command_fence && custom_command_fence_event.IsValid())
                {
                    if (SUCCEEDED(command_queue->Signal(
                        custom_command_fence.Get(),
                        custom_command_list_fence_value
                        )))
                    {
                        // Wait until the Signal has been processed.
                        if (SUCCEEDED(custom_command_fence->SetEventOnCompletion(
                            custom_command_list_fence_value,
                            custom_command_fence_event.Get()
                            )))
                        {
                            ...
                            custom_command_list_fence_value += 1;
                        }
                    }
                }
            }
            ...
            {
                if (nullptr != screen_size_resources)
                {
                    return screen_size_resources->GetBackBufferIndex();
                }
                return 0;
            }
                ...
                )
            {
                ...
                auto graphics_resource = graphics_memory->Allocate(
                    aligned_size,
                    in_alignment
                    );
                ...
                )
            {
                * in_pp_adapter = nullptr;
                Microsoft::WRL::ComPtr < IDXGIAdapter1 > adapter;
                for (UINT adapter_index = 0; SUCCEEDED(dxgi_factory->EnumAdapterByGpuPreference(
                    adapter_index,
                ...
                )
            {
                ...
                const int width = rc._right - rc._left;
                const int height = rc._bottom - rc._top;
                ...
                if ((nullptr != screen_size_resources) && (width == screen_size_resources->GetWidth()) && (height == 
                    screen_size_resources->GetHeight()))
                {
                    return false;
                }
                WaitForGpu();
                ...
                if (nullptr != screen_size_resources)
                {
                    fence_value = screen_size_resources->GetFenceValue();
                }
                screen_size_resources.reset();
                const unsigned int swap_flag = (in_options&c_allow_tearing) ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0u;
                screen_size_resources = std::make_unique < ScreenSizeResources > (
                    in_draw_system,
                    in_device,
                    in_dxgi_factory,
                    in_command_queue,
            ...
            void DeviceResources::MoveToNextFrame()
            {
                if (screen_size_resources)
                {
                    screen_size_resources->MoveToNextFrame(in_command_queue);
                }
            }
            ...
            void DeviceResources::Prepare(ID3D12GraphicsCommandList*&in_command_list)
            {
                if (nullptr != screen_size_resources)
                {
                    screen_size_resources->Prepare(in_command_list);
                }
                return;
            }
            ...
            {
                WaitForCustomCommand();
                DX::ThrowIfFailed(in_custom_command_allocator->Reset());
                DX::ThrowIfFailed(in_custom_command_list->Reset(
                    in_custom_command_allocator.Get(),
                    nullptr
                    ));
                ...
                if (custom_command_fence->GetCompletedValue() < custom_command_list_fence_value)
                {
                    DX::ThrowIfFailed(custom_command_fence->SetEventOnCompletion(
                        custom_command_list_fence_value,
                        custom_command_fence_event.Get()
                        ));
                    WaitForSingleObjectEx(
                ...
                custom_command_list_fence_value += 1;
                return in_custom_command_list.Get();
            }
            ...
            const bool DeviceResources::Present()
            {
                ...
                if (false == screen_size_resources->Present(
                    hr,
                    command_queue
                    ))
                {
                    ...
                    sprintf_s(
                        ...
                        static_cast < unsigned int > ((hr == DXGI_ERROR_DEVICE_REMOVED) ? device->GetDeviceRemovedReason
                            () : hr)
                        );
                ...
                if (graphics_memory)
                {
                    graphics_memory->Commit(command_queue.Get());
                }
                return true;
            }
            ...
            IRenderTarget* DeviceResources::GetRenderTargetBackBuffer()
            {
                if (nullptr != screen_size_resources)
                {
                    return screen_size_resources->GetRenderTargetBackBuffer();
                }
                return nullptr;
            }
